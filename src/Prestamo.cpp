...
void Prestamo::setDatos(int idPropietario_p, int numeroPrestamo_p, string tipoMoneda_p, double montoTotal_p, int cuotasTotal_p, double tasaInteres_p, int cuotasPagadas_p, string tipo_p){
    // Se actualizan/inicializan todos los atributos utilizados en este método con la info del cliente
    tipo = tipo_p;
    idPropietario = idPropietario_p;
    numeroPrestamo = numeroPrestamo_p;
    tipoMoneda = tipoMoneda_p;
    montoTotal = montoTotal_p;
    cuotasTotal = cuotasTotal_p;
    tasaInteres = tasaInteres_p;
    ...
    // Se actualiza el número de cuotas pagadas
    cuotasPagadas = cuotasPagadas_p;
}
...
void Prestamo::imprimirInfo(){
    ...
    cout << "Intereses pagados: " << interesesPagados << endl;
    cout << "Deuda pagada: " << deudaPagada << endl;
    cout << endl;
}
...
vector <string> Prestamo::generarInfoCuotas(){
    // Se declara una vector de strings que contendrá la información del reporte
    vector <string> reporte;
    // Se agrega un titulo para que quede claro el tipo de reporte de cuotas que se va a realizar
    reporte.push_back("----------------------------Reporte de cuotas----------------------------");
